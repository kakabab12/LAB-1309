...
**이 문서는 뭐 하는 곳?** 세상에 나온 VLA 모델들을 **무거운 순서로** 줄 세운 목록입니다.

**왜 무게가 중요해?** 모델이 크면 좋은 성능이 나오지만 **비싼 그래픽카드가 필요하고 느립니다.** 로봇은 실시간으로 움직여야 해서 느리면 못 씁니다.

**크기 감 잡기**
- **55B** = 550억 개 숫자 → 슈퍼컴퓨터급. 우리는 못 씀
- **7B** = 70억 → 비싼 서버 GPU 필요
- **0.45B** = 4억 5천만 → ⭐ **우리가 쓸 SmolVLA.** 개인 컴퓨터로 가능
- **0.2B** = 2억 → 아주 가벼움

**놀라운 사실**: 요즘은 **작은 모델이 큰 모델을 이깁니다.** 0.2B짜리가 7B짜리보다 점수가 높아요. 크기보다 **설계 방식**이 중요해졌거든요. 그래서 우리가 작은 모델로 연구해도 전혀 뒤처지지 않습니다.

**표가 어려우면**: 맨 아래 "우리 연구 기준 선택" 표만 보셔도 됩니다.
