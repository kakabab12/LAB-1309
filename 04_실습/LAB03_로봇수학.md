...
**뭐 하는 실습?** 로봇 팔의 위치·회전을 계산해보고, **동작이 얼마나 튀는지 숫자로 재는 법**을 만듭니다.

**여기서 만드는 것 중 제일 중요한 것: `jerk_score()`**
- 저크(jerk) = 동작이 **얼마나 갑작스럽게 튀는지** 나타내는 숫자
- 부드럽게 움직이면 작고, 홱 꺾이면 큽니다
- 우리 연구에서 **"내 방법이 더 부드럽다"를 증명할 때 쓰는 도구**예요
...
→ 그냥 갈아끼우면 709, 조금씩 섞으면 79. **9배 차이**입니다. 이게 우리 연구의 핵심 아이디어예요.

**회전 계산이 어려우면** scipy가 다 해줍니다. 함수 이름만 알면 돼요.
