...
**뭐 하는 실습?** AI 모델을 만드는 도구(PyTorch)를 손에 익힙니다. 손글씨 숫자를 알아맞히는 작은 AI를 **직접 만들어봅니다.**

**얼마나 어려워?** 파이썬을 쓰실 줄 알면 따라올 수 있습니다. 코드는 전부 적어뒀으니 **복사해서 돌려보고, 숫자를 바꿔가며 뭐가 변하는지** 보세요.

**여기서 진짜 배울 것 하나**
```
optimizer.zero_grad()   # 1. 이전 기록 지우고
loss.backward()         # 2. 얼마나 틀렸는지 거꾸로 계산하고
optimizer.step()        # 3. 숫자를 조금 고친다
```
이 세 줄이 **세상 모든 딥러닝 학습의 전부**입니다. 나머지는 이걸 크게 만든 것뿐이에요.

**GPU 없어도 됩니다.** 지금 이 컴퓨터에서 바로 시작하세요.
