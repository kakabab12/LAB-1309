...
**뭐 하는 실습?** ⭐ **우리 연구를 아주 작게 미리 해보는** 실습입니다. 가장 중요해요.

**상황을 그림으로**
```
모델: "동작 50개 줄게" → 로봇이 1번, 2번, 3번... 차례로 실행 중
사람: "아 잠깐, 다른 거 해줘"        ← 여기서 무슨 일이 일어나야 할까?
```

**세 가지 방법을 직접 코딩해서 비교합니다**
| 방법 | 뜻 | 결과(실제 측정) |
|---|---|---|
| 그냥 두기 | 받아놓은 50개 다 하고 나서 바꿈 | 8스텝이나 늦게 반응 |
| 즉시 교체 | 남은 걸 버리고 새 걸로 | 빠르지만 **동작이 튐** |
| **섞기** | 이전 것과 새 것을 조금씩 겹쳐서 | **1스텝만 늦고 부드러움** ⭐ |

이 표를 나중에 **진짜 SmolVLA로 다시 채우는 것**이 연구의 첫 결과가 됩니다.

**어려운 코드는 없습니다.** 리스트에서 하나씩 꺼내 쓰는 정도예요.
...
**실측 결과 (위 가짜 정책으로 직접 돌려본 값, t=12에 지시 변경)**

| 전략 | 반응 시작 | 지연 | 저크 |
|---|---|---|---|
| ① 그냥 두기 | t=20 | **8스텝** | 2116.6 |
| ② 즉시 교체 | t=12 | **0스텝** | 2116.6 |
| ③ 블렌딩 | t=13 | 1스텝 | **302.4** |

읽는 법: 즉시 교체는 **반응은 빠르지만 동작이 튀고**, 블렌딩은 **1스텝만 늦으면서 저크를 7배 줄입니다.**
가짜 정책이라 값 자체는 의미가 없지만, **이 표를 진짜 SmolVLA로 채우는 것**이 연구의 첫 결과가 됩니다.
