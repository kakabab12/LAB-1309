...
**뭐 하는 실습?** 요즘 AI(ChatGPT, VLA 전부)의 핵심 부품인 **Transformer**를 직접 만들어봅니다.

**왜 직접 만들어?** 논문 그림에 `attention`, `cross-attention` 같은 말이 계속 나오는데, **한 번 만들어보면 그 그림이 읽힙니다.** 안 만들어보면 평생 안 읽혀요.

**attention이 뭔데?** 쉽게 말하면 **"어디를 봐야 할지 고르는 장치"**입니다.
"빨간 블록을 집어"라는 말을 들었을 때, 사진 속 수백 군데 중 **빨간 블록 쪽을 더 많이 보게** 만드는 것.

**마지막에 만드는 것**: 사진 + 말 → 로봇 동작 50개를 내놓는 **아주 작은 VLA**. 진짜 SmolVLA와 구조가 같습니다. 크기만 작을 뿐이에요.
