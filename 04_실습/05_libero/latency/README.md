...
1. **저사양 문제는 실재하고, 우리 GPU 가 정확히 그 구간이다.** 실측 0.56초 = 11스텝. 지금까지의 모든 수치는 동기 실행이라 낙관적
2. **RTC 는 "지연 대응" 도구로는 맞다.** 전환 도구로는 아님
3. 전환 연구의 최종 평가는 **실제 추론 시간만큼 지연을 넣은 조건**에서 해야 공정함 → `--latency-steps 11` 로 재평가 중

## 지연을 줄이는 방법 (검토)

| 방법 | 기대 효과 | 비용 |
|---|---|---|
| **`n_action_steps` 늘리기** (10 → 25) | 추론 횟수가 2.5배 줄어 **추론 사이 간격(1.25초)이 지연(0.56초)보다 길어짐** → 계획이 바닥나지 않음 | 반응이 느려짐 (지시 변경에 최대 1.25초) |
| **RTC** | 측정됨: 12% → 28% | 전환 반응이 늦어짐 |
| 비동기 추론 (SmolVLA 논문 방식) | 멈춤(stop-and-go) 제거 | 구현 필요 |
| 입력 해상도 줄이기 (512 → 256) | VLM 토큰 수가 1/4 → 추론 시간 감소 | 정확도 하락 가능 |
| fp16 | Pascal 은 fp16 연산이 느려 **효과 적음** (1080 Ti 는 bf16 미지원) | - |
| VLM prefix 캐시 재사용 | 이미지가 크게 안 바뀌면 앞부분 재계산 생략 | 구현 필요, 정확도 영향 미지 |

**우선순위**: `n_action_steps` 조정이 가장 싸고 효과가 명확합니다. 반응 지연과의 맞바꿈을 재야 합니다.
⚠️ 단, 이건 **전환 실험과 정면으로 충돌**합니다 — 전환은 빨리 반응해야 하는데, 지연 대응은 천천히 반응하라고 합니다. **이 긴장 자체가 우리 연구의 핵심 문제**입니다.
...
- 태스크 4개, 조건당 10회
- 지연을 "고정 L 스텝 뒤 도착"으로 모사 — 실제로는 추론 시간이 흔들리고, 도착 시점이 제어 주기와 안 맞을 수 있음
