...
### ④ ⭐ 헛잡기 — 집는 동작은 나오는데 2~4cm 빗나간다

그리퍼가 **닫히는 순간** 목표 물체와 얼마나 떨어져 있는지 쟀습니다.

⚠️ 끝단 좌표의 기준점이 손가락 끝이 아니라 손목이라 **상수 오차**가 있습니다.
그래서 **교란 없이 성공한 집기의 거리(5.7cm)를 '오차 0'** 으로 놓고 비교했습니다.

| 조건 | 위치 오차 | 손목 회전 이탈 |
|---|---|---|
| **교란 없이 성공한 집기** (기준, n=40) | **0.0cm** | **6.9도** |
| 전환 뒤 A8→B3 (n=11) | **+3.1cm** | **15.7도** |
| 전환 뒤 A1→B3 (n=10) | +1.8cm | 16.2도 |
| 전환 뒤 A4→B3 (n=9) | +3.6cm | 13.2도 |

**집는 동작 자체는 제대로 나옵니다.** 다만 **2~4cm 빗나간 곳에서, 손목이 2배 이상 틀어진 채**
닫습니다. 그릇 지름을 생각하면 그만큼이면 놓칩니다.

> 이것이 "5cm 앞까지 가는데 왜 못 집는가"의 답입니다 —
> **가기는 가는데, 마지막 정렬이 안 됩니다.**
...
| 교란 | **집어야 함** (T1·T8·T9 평균) | **집을 필요 없음** (T0 서랍) | 차이 |
|---|---|---|---|
| **0cm** | **80%** | **80%** | **0%p** |
| 12cm | 20% | 50% | +30%p |
| 20cm | 11% | 50% | +39%p |
...
| 태스크 | 0cm | 12cm | 20cm | 27cm |
|---|---|---|---|---|
| T1 그릇→스토브 | 100% | 30% | 10% | **0%** |
| T8 그릇→접시 | 90% | 30% | 22% | 11% |
| T9 와인→선반 | 50% | 0% | 0% | 0% |
| **T0 서랍 열기** | 80% | 50% | 50% | **62%** |

**교란이 없을 때는 정확히 같은 80% 입니다.**
그런데 교란을 주면 **집는 쪽만** 무너지고, 안 집는 쪽은 **사실상 평평합니다**(50~62%).

> 서랍 열기는 팔을 27cm 옮겨 놓고 시작해도 **62%** 나 해냅니다.
> 같은 교란에서 물체 집기는 **4%** 입니다.

**교란된 자세에서 무너지는 것은 '이동'이 아니라 '집기'입니다.**
이것이 전환 실험의 30배 차이(물체 1.4% vs 가구·기구 44.6%)를 그대로 설명합니다.
