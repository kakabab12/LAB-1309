...
그릇을 든 순간 지시를 바꿉니다. 왼쪽은 "서랍을 열어라", 오른쪽은 "그릇을 서랍에 넣어라".

![전환 성공 vs 실패](img/switch_ok_vs_bad.gif)

- **왼쪽**: 그릇을 놓고 서랍으로 가서 연다 → **성공 (60%)**
- **오른쪽**: 그릇을 놓은 뒤 **그 그릇으로 돌아가지 않는다** → **실패 (0%)**
  놓은 자리에서 **16cm 밖**에 머물며 허공을 잡습니다

### ② 같은 교란(27cm)에서 시작할 때

팔을 27cm 옮겨 놓고 시작합니다. 전환 시점의 이탈과 같은 크기입니다.

![서랍 열기 vs 그릇 집기](img/pose27_open_vs_grasp.gif)

- **왼쪽 (서랍 열기)**: 팔이 멀리 있어도 손잡이를 찾아 연다 → **62%**
- **오른쪽 (그릇 집기)**: 같은 교란인데 못 집는다 → **11%**
