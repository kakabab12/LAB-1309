...
def main():
    def label(a):
        """통제변수가 다른 실행은 이름을 달리해 기준선이 섞이지 않게 한다."""
        s = a["strategy"]
        if a.get("dither_escape", "off") != "off":
            return f"흔들기 {a['dither_escape']}"
        if a.get("switch_noise_seed", -1) >= 0:
            return f"시드 {a['switch_noise_seed']}"
        if a.get("switch_n_action_steps", 0):
            return f"{s} sn{a['switch_n_action_steps']}"
        if a.get("latency_steps", 0):
            return f"{s} lat{a['latency_steps']}"
        return s

    rows = collect("outputs/switch", label)
    ...
    order = ["flush", "keep", "blend", "rtc", "flush_rtc", "bon", "rollback", "finish_a",
             "흔들기 lift", "흔들기 reverse", "흔들기 random", "흔들기 home",
             "release", "ret_rot", "ret_pos", "retreat", "LoRA 1차 flush", "LoRA 2차 flush"]
