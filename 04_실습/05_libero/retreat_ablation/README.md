...
### 영상: 위치만 복구 vs 회전만 복구 (같은 에피소드)

![위치 복구 vs 회전 복구](results/ablation_pos_vs_rot.gif)

8→7(그릇을 접시에 → 스토브 켜기), 잡은 직후 전환, 에피소드 0. 왼쪽은 위치를 복구해서 스토브를 켜고, 오른쪽은 회전만 복구한 뒤 그릇을 스토브 위에 둔 채 손잡이를 찾지 못합니다. 색 막대: 파랑 A, 회색 스크립트 복귀, 주황 B, 초록 A 재개.
...
> ⚠️ **2026-09-17 수정**: 이후 [가설 검증](../hypothesis_check/README.md)에서, 실제 전환 112회를 보니 **전환 순간의 위치 거리로는 성공이 예측되지 않았습니다** (r = +0.14). 이 문서의 "위치가 핵심"은 **"초기 위치로 되돌리는 동작이 효과적"** 까지만 맞고, "위치가 멀어서 실패한다"는 인과는 확인되지 않았습니다. 되돌리기에는 물체 내려놓기·정지·장면 정리 효과도 섞여 있습니다.
