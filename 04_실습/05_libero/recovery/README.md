...
| 시도 | 결과 | 배운 것 |
|---|---|---|
| 흔들기 4방향 | ❌ 변화 없음 | 움직임을 주는 것만으로는 안 됨. **PIP 의 이득이 우리 설정에서 재현되지 않음** |
| `rollback` | ❌ 더 나쁨 (p=0.064) | 구현은 정확(오차 2.4cm)하나 **되돌아가는 거리가 7.7cm 뿐** |
| 거리-성능 관계 | ⏳ 측정 중 | 부분 이동은 무효, **도착**이 조건으로 보임 |

**세 번의 음성 결과가 문제를 좁혔습니다.** "적당히 움직여서 분포 안으로 돌아가기"는 안 됩니다.
남은 길은 ① 완전 복귀에 준하는 부분 복귀(측정 중) ② **정책 자체를 낯선 자세에서도 작동하게 학습**하는 것입니다.
