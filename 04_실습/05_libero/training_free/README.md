...
**그래서 `rollback`(최근 동작 역순으로 되돌려 집었던 자리에 놓기)이 중요합니다.** 초기 자세로 가지 않으면서 물체는 원래 자리에 놓으므로, **제약을 지키면서 A 재개도 가능한** 유일한 후보입니다. → [방법_총정리](../방법_총정리.md)
...
→ 다음: `rollback` 전략(제약을 지키며 물체를 되돌리기), 오라클 Best-of-N(후보 선택의 상한), [조종 가능성 CMI](../../../02_논문노트/ReSteer.md)
