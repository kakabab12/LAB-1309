...
> ⚠️ **2026-09-18 정정.** 처음에는 "되돌려 놓으면 물체가 원래 자리에 있어서"라고 적었는데 **틀렸습니다.**
> `retreat` 코드를 다시 읽어 보니 물체를 **전환 지점에서 그냥 내려놓고 팔만 초기 자세로** 갑니다
> (접촉할 때까지 하강 → 그리퍼 열기 → 들어올리기 → 초기 자세 복귀). 물체는 원래 자리로 가지 않습니다.
>
> 그러므로 재개가 되는 이유는 **팔이 초기 자세로 돌아가서**입니다 —
> [B 성공의 조건과 같습니다](../recovery/README.md)(초기 위치 7cm 안).
> `ret_pos`(9%) 와 `retreat`(62%) 의 차이도 **손목 회전**을 되돌리는지의 차이입니다.

- 그 자리에서 전환하면 팔이 초기 위치에서 27cm 떨어진 채 A2 를 시작합니다 → 실패
- 되돌려 놓으면 팔이 초기 자세에서 시작합니다 → A 의 물체가 옮겨져 있어도 62% 는 해냅니다
- 다만 **재개는 B 보다 더 어렵습니다**: 팔이 7.4cm 까지 돌아간 `ret_part` f=0.75 는 B 67% 인데 재개는 8% 입니다.
  물체까지 옮겨져 있어 **두 가지 이탈이 겹치기** 때문으로 보입니다
